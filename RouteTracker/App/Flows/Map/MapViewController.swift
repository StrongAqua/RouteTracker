...
class MapViewController: UIViewController {
        ...
        let subscription =
            ...
            .subscribe({ [weak self] event in
                ...
                if self.isTrackingStarted {
                    ...
                    debugPrint("LOCATION: Location updated, \(self.routePath.count())")
                } else {
                ...
                self.showMarker(position: location.coordinate)
            })
    ...
    func showMarker(position: CLLocationCoordinate2D) {
        let image = UIImage(
            contentsOfFile: self.getDocumentsDirectory().appendingPathComponent("marker.png").path
        )
        ...
        // marker.icon = image //GMSMarker.markerImage(with: .blue)
        marker.map = mapView
        self.marker = marker
    }
}
